...
pub fn build_accounts(input: &KaminoWithdrawInput) -> Vec<AccountMeta> {
    let mut accounts = vec![
        ...
        AccountMeta::new(input.reserve_source_collateral, false),
...
pub async fn resolve(
    ...
    let user_destination_liquidity_account = rpc.get_account(user_destination_liquidity).await?;
    if user_destination_liquidity_account.owner != reserve_snapshot.liquidity_token_program {
        return Err(crate::error::ClientError::InvalidAccountData(format!(
            "token account {} is owned by {}, expected {}",
            user_destination_liquidity,
            user_destination_liquidity_account.owner,
            reserve_snapshot.liquidity_token_program
        )));
    }

    let (token_account_mint, token_account_owner) =
        read_token_account_mint_and_owner(&user_destination_liquidity_account.data)?;
    if token_account_owner != *user {
        return Err(crate::error::ClientError::InvalidAccountData(format!(
            "token account {} owner {} does not match user {}",
            user_destination_liquidity, token_account_owner, user
        )));
    }
    ...
    let input = KaminoWithdrawInput {
        ...
        reserve_source_collateral: reserve_snapshot.collateral_supply_vault,
        ...
        token_program: crate::TOKEN_PROGRAM_ID,
...
mod tests {
    ...
    fn build_accounts_keeps_program_first_and_preserves_tail() {
        let input = KaminoWithdrawInput {
            ...
            withdraw_reserve: Address::new_from_array([5; 32]),
            reserve_liquidity_mint: Address::new_from_array([6; 32]),
            reserve_source_collateral: Address::new_from_array([7; 32]),
            reserve_collateral_mint: Address::new_from_array([8; 32]),
            reserve_liquidity_supply: Address::new_from_array([9; 32]),
            user_destination_liquidity: Address::new_from_array([10; 32]),
            placeholder_user_destination_collateral: KAMINO_LEND_PROGRAM_ID,
            token_program: crate::TOKEN_PROGRAM_ID,
